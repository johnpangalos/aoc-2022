...
pub fn main() !void {
    ...
    while (iter.next()) |line| {
        ...
        var popped = std.ArrayList(u8).init(std.heap.page_allocator);
        defer popped.deinit();

        while (move > 0) {
            try popped.append(stacks.items[from - 1].pop());
            move -= 1;
        }

        while (popped.items.len > 0) {
            try stacks.items[to - 1].append(popped.pop());
        }
    }
